...
import EquipmentForm, { isValidEquipmentDate } from '../components/EquipmentForm';
import { getEquipmentStatus } from '../utils/equipment';
...
export default function EquipmentDetailScreen({ route, navigation }: any) {
  ...
  const saveService = async () => {
    ...
    if (!isValidEquipmentDate(serviceForm.serviceDate) || (serviceForm.nextDueDate && !isValidEquipmentDate(serviceForm.nextDueDate)) || !serviceForm.serviceType.trim()) {
      setServiceError('Enter a service type and valid dates as YYYY-MM-DD.');
      return;
    }
    if (serviceForm.cost.trim() && (!Number.isFinite(Number(serviceForm.cost)) || Number(serviceForm.cost) < 0)) {
      setServiceError('Enter a valid non-negative cost.');
      return;
    }
    setServiceError('');
    ...
    try {
      ...
      setServiceError('Could not save the service record. Please try again.');
    } finally {
  ...
  if (error && !equipment) return <View style={styles.center}><Text accessibilityRole="alert" style={styles.errorText}>{error}</Text><TouchableOpacity accessibilityRole="button" style={styles.retryButton} onPress={loadDetail}><Text style={styles.retryText}>Retry</Text></TouchableOpacity></View>;
  if (!equipment) return null;

  return (
    <View style={styles.screen}>
      <ScrollView contentContainerStyle={styles.content}>
        <View style={styles.header}><TouchableOpacity accessibilityRole="button" accessibilityLabel="Back to equipment" onPress={() => navigation.goBack()}><Text style={styles.backText}>‹ Equipment</Text></TouchableOpacity><TouchableOpacity accessibilityRole="button" style={styles.editButton} onPress={() => setEditVisible(true)}><Text style={styles.editText}>Edit item</Text></TouchableOpacity></View>
        <View style={styles.hero}><View style={styles.heroCopy}><Text style={styles.eyebrow}>{equipment.category}</Text><Text style={styles.title}>{equipment.name}</Text><Text style={styles.subtitle}>{[equipment.brand, equipment.model].filter(Boolean).join(' ') || 'Brand and model not specified'}</Text></View><View style={styles.statusBadge}><Text style={styles.statusText}>{getEquipmentStatus(equipment) === 'service_due' ? 'Service due' : equipment.condition === 'retired' ? 'Retired' : 'Ready'}</Text></View></View>
        {error ? <Text accessibilityRole="alert" style={styles.inlineError}>{error}</Text> : null}
        <View style={styles.tabs}>{(['overview', 'maintenance', 'packing'] as Tab[]).map((value) => <TouchableOpacity accessibilityRole="button" aria-selected={tab === value} accessibilityState={{ selected: tab === value }} key={value} onPress={() => setTab(value)} style={[styles.tab, tab === value && styles.tabActive]}><Text style={[styles.tabText, tab === value && styles.tabTextActive]}>{value === 'overview' ? 'Overview' : value === 'maintenance' ? 'Maintenance' : 'Packing list'}</Text></TouchableOpacity>)}</View>

        {tab === 'overview' ? <View style={styles.panel}><View style={styles.infoGrid}><View style={styles.infoCell}><Text style={styles.infoLabel}>Next service</Text><Text style={styles.infoValue}>{formatDate(equipment.nextServiceDate)}</Text></View><View style={styles.infoCell}><Text style={styles.infoLabel}>Serial number</Text><Text style={styles.infoValue}>{equipment.serialNumber || 'Not specified'}</Text></View><View style={styles.infoCell}><Text style={styles.infoLabel}>Purchased</Text><Text style={styles.infoValue}>{formatDate(equipment.purchaseDate)}</Text></View></View><Text style={styles.panelTitle}>Notes</Text><Text style={styles.notes}>{equipment.notes || 'No notes for this item.'}</Text><View style={styles.usageCard}><Text style={styles.panelTitle}>Packing contexts</Text><Text style={styles.notes}>{packs.length ? `${packs.length} trip or dive context${packs.length === 1 ? '' : 's'} linked.` : 'This item has not been added to a packing list yet.'}</Text><TouchableOpacity accessibilityRole="button" style={styles.secondaryButton} onPress={() => setTab('packing')}><Text style={styles.secondaryButtonText}>Manage packing</Text></TouchableOpacity></View></View> : null}

        {tab === 'maintenance' ? <View style={styles.panel}><View style={styles.panelHeader}><Text style={styles.panelTitle}>Service history</Text><TouchableOpacity accessibilityRole="button" style={styles.primaryButton} onPress={openServiceForm}><Text style={styles.primaryButtonText}>+ Add service</Text></TouchableOpacity></View>{records.length ? records.map((record) => <View key={record.id} style={styles.recordCard}><View style={styles.recordHeader}><Text style={styles.recordType}>{record.serviceType}</Text><Text style={styles.recordDate}>{formatDate(record.serviceDate)}</Text></View><Text style={styles.recordMeta}>{record.provider || 'No provider'}{record.cost !== null && record.cost !== undefined ? ` · €${record.cost}` : ''}</Text><Text style={styles.recordNotes}>{record.notes || 'No notes added.'}</Text><View style={styles.recordFooter}><Text style={styles.nextDue}>Next due: {formatDate(record.nextDueDate)}</Text><TouchableOpacity accessibilityRole="button" accessibilityLabel={`Remove ${record.serviceType} record`} onPress={() => removeRecord(record)}><Text style={styles.deleteText}>Remove</Text></TouchableOpacity></View></View>) : <View style={styles.emptyCard}><Text style={styles.emptyTitle}>No service history yet</Text><Text style={styles.emptyText}>Record inspections and repairs to keep your kit safe.</Text><TouchableOpacity accessibilityRole="button" style={styles.secondaryButton} onPress={openServiceForm}><Text style={styles.secondaryButtonText}>Add first record</Text></TouchableOpacity></View>}</View> : null}

        {tab === 'packing' ? <View style={styles.panel}><Text style={styles.panelTitle}>Add this item to a trip or dive</Text><Text style={styles.helper}>Choose a context to track whether this equipment is packed.</Text><View style={styles.contextRow}>{contexts.length ? contexts.map((context) => <TouchableOpacity accessibilityRole="button" aria-selected={selectedContext?.key === context.key} accessibilityState={{ selected: selectedContext?.key === context.key }} key={context.key} style={[styles.contextChip, selectedContext?.key === context.key && styles.contextChipActive]} onPress={() => setContextKey(context.key)}><Text style={[styles.contextLabel, selectedContext?.key === context.key && styles.contextLabelActive]}>{context.label}</Text><Text style={[styles.contextDetail, selectedContext?.key === context.key && styles.contextDetailActive]}>{context.detail}</Text></TouchableOpacity>) : <Text style={styles.emptyText}>Create a planned dive or trip first.</Text>}</View>{selectedContext ? <View style={styles.packPanel}><Text style={styles.packContextTitle}>{selectedContext.label}</Text><Text style={styles.notes}>{selectedContext.detail}</Text><TouchableOpacity accessibilityRole="button" style={[styles.packToggle, selectedPack?.packed && styles.packToggleActive]} onPress={togglePack}><Text style={[styles.packToggleText, selectedPack?.packed && styles.packToggleTextActive]}>{selectedPack?.packed ? '✓ Packed for this context' : selectedPack ? 'Mark as packed' : 'Add to packing list'}</Text></TouchableOpacity></View> : null}</View> : null}
      </ScrollView>

      {editVisible ? <EquipmentForm editing={equipment} onClose={() => setEditVisible(false)} onSaved={loadDetail} /> : null}
      <Modal visible={serviceModal} transparent animationType="slide" onRequestClose={() => setServiceModal(false)}><View style={styles.modalBackdrop}><View style={styles.modalCard}><ScrollView keyboardShouldPersistTaps="handled"><View style={styles.modalHeader}><Text style={styles.modalTitle}>Add service record</Text><TouchableOpacity accessibilityRole="button" accessibilityLabel="Close service form" onPress={() => setServiceModal(false)}><Text style={styles.closeText}>×</Text></TouchableOpacity></View><Text style={styles.fieldLabel}>Service date *</Text><TextInput accessibilityLabel="Service date" value={serviceForm.serviceDate} onChangeText={(value) => setServiceForm((current) => ({ ...current, serviceDate: value }))} placeholder="YYYY-MM-DD" style={styles.input} /><Text style={styles.fieldLabel}>Service type *</Text><TextInput accessibilityLabel="Service type" value={serviceForm.serviceType} onChangeText={(value) => setServiceForm((current) => ({ ...current, serviceType: value }))} placeholder="Annual inspection" style={styles.input} /><Text style={styles.fieldLabel}>Next due date</Text><TextInput accessibilityLabel="Next due date" value={serviceForm.nextDueDate} onChangeText={(value) => setServiceForm((current) => ({ ...current, nextDueDate: value }))} placeholder="YYYY-MM-DD" style={styles.input} /><View style={styles.formRow}><View style={styles.formHalf}><Text style={styles.fieldLabel}>Provider</Text><TextInput accessibilityLabel="Provider" value={serviceForm.provider} onChangeText={(value) => setServiceForm((current) => ({ ...current, provider: value }))} placeholder="Dive centre" style={styles.input} /></View><View style={styles.formHalf}><Text style={styles.fieldLabel}>Cost</Text><TextInput accessibilityLabel="Cost" value={serviceForm.cost} onChangeText={(value) => setServiceForm((current) => ({ ...current, cost: value }))} placeholder="0" keyboardType="decimal-pad" style={styles.input} /></View></View><Text style={styles.fieldLabel}>Notes</Text><TextInput accessibilityLabel="Service notes" value={serviceForm.notes} onChangeText={(value) => setServiceForm((current) => ({ ...current, notes: value }))} multiline placeholder="What was checked?" style={[styles.input, styles.notesInput]} />{serviceError ? <Text accessibilityRole="alert" style={styles.inlineError}>{serviceError}</Text> : null}<View style={styles.modalActions}><TouchableOpacity accessibilityRole="button" style={styles.cancelButton} onPress={() => setServiceModal(false)}><Text style={styles.cancelText}>Cancel</Text></TouchableOpacity><TouchableOpacity accessibilityRole="button" disabled={savingService} style={[styles.primaryButton, savingService && styles.disabledButton]} onPress={saveService}><Text style={styles.primaryButtonText}>{savingService ? 'Saving…' : 'Save record'}</Text></TouchableOpacity></View></ScrollView></View></View></Modal>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  tabs: { flexDirection: 'row', flexWrap: 'wrap', gap: 7, borderBottomWidth: 1, borderBottomColor: '#dbe6ee', marginTop: 20, marginBottom: 16 },
  ...
  infoCell: { flexGrow: 1, flexShrink: 1, flexBasis: 180, minWidth: 0, backgroundColor: '#f5fbfd', borderRadius: 11, padding: 12 },
  ...
  panelHeader: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', gap: 12, marginBottom: 16 },
  ...
  recordFooter: { flexDirection: 'row', flexWrap: 'wrap', gap: 10, justifyContent: 'space-between', alignItems: 'center', marginTop: 13, paddingTop: 9, borderTopWidth: 1, borderTopColor: '#edf1f3' },
  ...
  contextRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 9, paddingVertical: 15 },
  contextChip: { flexGrow: 1, flexShrink: 1, flexBasis: 180, minWidth: 0, borderWidth: 1, borderColor: '#b8d8ee', borderRadius: 12, padding: 11, backgroundColor: '#fff' },
