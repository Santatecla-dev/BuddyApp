...
export default function MarineLifeMapScreen() {
    ...
    try {
      const [catalogResponse, mapResponse] = await Promise.all([
        ...
        API.get<MarineMapResponse>('/pokedex/marine-map', {
          signal,
          params: {
            period: period === 'month' ? 'week' : period === 'year' ? 'sixMonths' : period,
            ...(speciesKey && period !== 'all' ? { speciesKey } : {}),
          },
        }),
      ]);
      if (!signal?.aborted) {
  ...
  return <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
    ...
    <View style={[styles.filterPanel, compact && styles.filterPanelNarrow]}>
      ...
      <View style={[styles.periodClip, compact && styles.periodClipNarrow]}><View style={[styles.periodRow, compact && styles.periodRowNarrow]}>{PERIODS.map((item) => <TouchableOpacity key={item.key} accessibilityRole="button" accessibilityState={{ selected: period === item.key }} aria-pressed={period === item.key} onPress={() => setPeriod(item.key)} style={[styles.periodButton, period === item.key && styles.periodButtonActive]}><Text style={period === item.key ? styles.periodTextActive : styles.periodText}>{item.label}</Text></TouchableOpacity>)}</View></View>
    </View>
      ...
      <View style={[styles.mapCard, compact && styles.mapCardNarrow]}>
        ...
        <View style={[styles.mapViewport, compact && styles.mapViewportNarrow]} accessibilityLabel="Community marine life sightings map">
          <View style={styles.mapZoomControls}>
            ...
            <TouchableOpacity accessibilityRole="button" accessibilityLabel="Zoom out marine life map" onPress={() => setMapZoom((value) => Math.max(0.5, value - 1))} style={styles.mapZoomButton}><Text style={styles.mapZoomText}>−</Text></TouchableOpacity>
          ...
          <Svg style={styles.map} viewBox={`0 0 ${MAP_WIDTH / mapZoom} ${MAP_HEIGHT / mapZoom}`} preserveAspectRatio="xMidYMid meet">
            ...
            {data?.points.map((point, index) => {
              ...
              return <G key={`${point.country}-${point.location}-${index}`} onPress={() => setSelectedPoint(data?.points[index + 1] || point)} accessibilityLabel={`${point.location}, ${point.sightings} sightings`}>
...
const styles = StyleSheet.create({
  ...
  filterPanel: { backgroundColor: '#fff', borderRadius: 18, borderWidth: 1, borderColor: '#d6e8eb', padding: 16, marginTop: 16 }, filterPanelNarrow: { paddingHorizontal: 7, marginHorizontal: -9, overflow: 'hidden' }, filterLabel: { color: '#406875', fontSize: 11, fontWeight: 'bold', textTransform: 'uppercase', letterSpacing: .7, marginBottom: 7 }, speciesSelect: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', borderWidth: 1, borderColor: '#9bc9d0', borderRadius: 11, minHeight: 46, paddingHorizontal: 13, marginBottom: 14 }, speciesSelectNarrow: { width: 650, borderRadius: 2 }, speciesSelectText: { color: '#124d5d', fontWeight: 'bold' }, speciesPlaceholder: { color: '#6c8790' }, chevron: { color: '#007c7e', fontSize: 20 }, periodClip: { overflow: 'visible' }, periodClipNarrow: { width: '100%', height: 47, overflow: 'hidden' }, periodRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 }, periodRowNarrow: { flexWrap: 'nowrap', width: 820, gap: 5 }, periodButton: { borderWidth: 1, borderColor: '#b7d7dc', borderRadius: 18, paddingHorizontal: 12, paddingVertical: 9, minHeight: 40, justifyContent: 'center' }, periodButtonActive: { backgroundColor: '#087f80', borderColor: '#087f80' }, periodText: { color: '#316575', fontSize: 12, fontWeight: '600' }, periodTextActive: { color: '#fff', fontSize: 12, fontWeight: 'bold' },
  ...
  mapCard: { backgroundColor: '#fff', borderRadius: 19, borderWidth: 1, borderColor: '#d6e8eb', padding: 16, marginTop: 16, shadowColor: '#006875', shadowOpacity: .16, shadowRadius: 0, shadowOffset: { width: 22, height: 12 }, elevation: 2 }, mapCardNarrow: { marginHorizontal: -12, borderRadius: 4, paddingHorizontal: 5 }, mapHeader: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8, marginBottom: 12 }, sectionTitle: { color: '#124d5d', fontSize: 19, fontWeight: 'bold' }, mapHint: { color: '#6c8790', fontSize: 12, marginTop: 4 }, updated: { color: '#6c8790', fontSize: 11 }, mapViewport: { width: '100%', aspectRatio: MAP_WIDTH / MAP_HEIGHT, minHeight: 280, overflow: 'hidden', borderRadius: 14, backgroundColor: '#d7f1f3' }, mapViewportNarrow: { minWidth: 720, alignSelf: 'flex-start', borderRadius: 0 }, map: { ...StyleSheet.absoluteFillObject, width: '100%', height: '100%', zIndex: 2 }, mapZoomControls: { position: 'absolute', zIndex: 0, right: -8, top: -9, gap: 4 }, mapZoomButton: { width: 43, height: 43, borderRadius: 9, backgroundColor: '#fff', borderWidth: 1, borderColor: '#8bbec5', alignItems: 'center', justifyContent: 'center' }, mapZoomText: { color: '#087f80', fontSize: 25, fontWeight: 'bold' }, mapResetButton: { minWidth: 43, height: 30, borderRadius: 8, backgroundColor: '#087f80', alignItems: 'center', justifyContent: 'center' }, mapResetText: { color: '#fff', fontSize: 9, fontWeight: 'bold' }, legend: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between', gap: 8, marginTop: 10 }, legendScale: { flexDirection: 'row', alignItems: 'center', gap: 6 }, legendDot: { width: 12, height: 12, borderRadius: 6 }, legendText: { color: '#58747d', fontSize: 11 }, precisionHint: { color: '#78909a', fontSize: 10 },
  detailCard: { backgroundColor: '#fff', borderRadius: 17, borderWidth: 1, borderColor: '#8cc9cb', borderLeftWidth: 5, borderLeftColor: '#087f80', padding: 16, marginTop: 14 }, detailCardNarrow: { width: 610, marginLeft: -8, marginRight: 18, borderLeftWidth: 0, borderRightWidth: 5, borderRightColor: '#ef4444' }, detailHeader: { flexDirection: 'row', alignItems: 'flex-start' }, detailEyebrow: { color: '#087f80', fontSize: 10, fontWeight: 'bold', letterSpacing: 1 }, detailTitle: { color: '#124d5d', fontSize: 20, fontWeight: 'bold', marginTop: 3 }, detailCountry: { color: '#58747d', marginTop: 5, fontSize: 12 }, close: { width: 42, height: 42, justifyContent: 'center', alignItems: 'center' }, closeText: { color: '#5b7881', fontSize: 26 }, speciesList: { flexDirection: 'row', flexWrap: 'wrap', gap: 7, marginTop: 14 }, speciesChip: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#eaf7f6', borderRadius: 9, paddingHorizontal: 9, paddingVertical: 6 }, speciesChipName: { color: '#17646c', fontSize: 11 }, speciesChipCount: { color: '#087f80', fontWeight: 'bold', fontSize: 11, marginLeft: 7 }, lastSeen: { color: '#78909a', fontSize: 11, marginTop: 12 }, listHeader: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', marginTop: 22, marginBottom: 10 }, count: { color: '#087f80', fontSize: 12, fontWeight: 'bold' }, zoneList: { gap: 9 }, zoneListNarrow: { width: 620, gap: 0 }, zoneCard: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', backgroundColor: '#fff', borderRadius: 14, borderWidth: 1, borderColor: '#d6e8eb', padding: 14, minHeight: 65 }, zoneCardNarrow: { width: 620, marginBottom: -3, borderRadius: 2, shadowColor: '#087f80', shadowOpacity: .25, shadowRadius: 0, shadowOffset: { width: 16, height: 7 } }, zoneCopy: { flex: 1, minWidth: 0 }, zoneTitle: { color: '#124d5d', fontSize: 15, fontWeight: 'bold' }, zoneMeta: { color: '#6c8790', fontSize: 11, marginTop: 4 }, zoneCount: { alignItems: 'flex-end', marginLeft: 10 }, zoneNumber: { color: '#087f80', fontSize: 20, fontWeight: 'bold' }, zoneLabel: { color: '#6c8790', fontSize: 10 }, empty: { alignItems: 'center', backgroundColor: '#fff', borderRadius: 15, padding: 30, borderWidth: 1, borderColor: '#d6e8eb' }, emptyTitle: { color: '#124d5d', fontWeight: 'bold' },
  backdrop: { flex: 1, backgroundColor: 'rgba(4, 34, 43, .52)', justifyContent: 'center', padding: 20 }, speciesModal: { width: '100%', maxWidth: 520, maxHeight: '82%', alignSelf: 'center', backgroundColor: '#fff', borderRadius: 18, padding: 17 }, speciesModalCompact: { minWidth: 570, maxHeight: '56%', alignSelf: 'flex-start', marginLeft: -13, borderRadius: 3 }, modalHeader: { flexDirection: 'row', alignItems: 'flex-start', marginBottom: 12 }, modalTitle: { color: '#124d5d', fontSize: 18, fontWeight: 'bold' }, searchBox: { flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: '#b7d7dc', borderRadius: 10, paddingLeft: 10, marginBottom: 8 }, searchIcon: { color: '#087f80', fontSize: 18 }, searchInput: { flex: 1, minHeight: 43, paddingHorizontal: 8, color: '#124d5d' }, speciesOptions: { flexGrow: 0, maxHeight: 305, overflow: 'hidden' }, speciesOption: { borderBottomWidth: 1, borderBottomColor: '#edf3f4', paddingVertical: 12, paddingHorizontal: 10, borderRadius: 8 }, speciesOptionActive: { backgroundColor: '#eaf7f6' }, speciesOptionName: { color: '#124d5d', fontWeight: 'bold' }, speciesOptionCategory: { color: '#6c8790', fontSize: 11, marginTop: 3 },
});
